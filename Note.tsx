...
// FIXED: Improved typewriter hook with active state
const useTypewriter = (
  text: string,
  speed: number = 50,
  active: boolean = true
...
const Note: React.FC<{
  ...
  // Text content
  const textContent = [
    "It's the year 2025, and you serve on a vision-driven crew of the upcoming orbital spaceship in Neo-City.",
    "Nova, your engineering specialist, signals she wants to talk after a tough week full of pressure and errors.",
  ];

  //  FIXED: Better state management for paragraphs
  const [showSecondParagraph, setShowSecondParagraph] = useState(false);

  // FIXED: Track when second paragraph should start typing
  const [secondParagraphActive, setSecondParagraphActive] = useState(false);

  // Typewriter instances
  const {
    displayText: displayText1,
    isComplete: isComplete1,
    skipTypewriter: skipTypewriter1,
    resetTypewriter: resetTypewriter1,
  } = useTypewriter(textContent[0], 15, true);

  const {
    displayText: displayText2,
    isComplete: isComplete2,
    skipTypewriter: skipTypewriter2,
    resetTypewriter: resetTypewriter2,
  } = useTypewriter(textContent[1], 15, secondParagraphActive);

  //  FIXED: Show second paragraph after first is complete
  useEffect(() => {
    if (isComplete1 && !showSecondParagraph) {
      const timer = setTimeout(() => {
        setShowSecondParagraph(true);
        // Start second paragraph typewriter after a small delay
        setTimeout(() => {
          setSecondParagraphActive(true);
        }, 50);
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [isComplete1, showSecondParagraph]);

  // Keyboard event listener for skipping current active paragraph
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (
        ...
        if (!isComplete1) {
          // If first paragraph is still typing, skip it
          skipTypewriter1();
        } else if (showSecondParagraph && !isComplete2) {
          // If second paragraph is visible and still typing, skip it
          skipTypewriter2();
        }
      }
    };
  ...
  // Separate handlers for each paragraph
  const handleSkipFirstParagraph = useCallback(() => {
    skipTypewriter1();
  }, [skipTypewriter1]);

  const handleSkipSecondParagraph = useCallback(() => {
    skipTypewriter2();
  }, [skipTypewriter2]);

  // FIXED: Reset typewriter when navigating
  const handleNavigation = useCallback(
    (direction: "left" | "right") => {
      resetTypewriter1();
      resetTypewriter2();
      setShowSecondParagraph(false);
      setSecondParagraphActive(false);
      onNavigate?.(direction);
    },
    [resetTypewriter1, resetTypewriter2, onNavigate]
  );
  ...
  const NavigationArrow: React.FC<{
    ...
    return (
      ...
      >
        {direction === "left" ? (
          l<ChevronLeft className="!w-4 !h-4 text-white" />
        ) : (
    ...
    return (
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6 lg:pb-8 relative h-full flex">
            {/* Top Navigation Bar */}
            <div className="absolute top-0 left-0 right-0 h-22 flex items-center justify-between bg-cover bg-center bg-no-repeat px-4 sm:px-6 lg:px-8" style={{ backgroundImage: "url('/images/top-nav-bg.png')" }}>            <div className="flex items-center gap-6">
            ...
            <div
                className="flex flex-col gap-5 max-h-fit max-w-[54.875rem]  overflow-y-auto overflow-x-hidden h-full w-full items-center justify-start rounded-[2.375rem] relative px-10 py-8 backdrop-filter backdrop-blur-2xl"
                style={{
            ...
            >
                ...
                {/* Note Content Area */}
                <div className="flex-1 w-full flex items-center justify-center px-5 py-4 relative z-0">
                <div className="w-full max-w-4xl mx-auto">
                    ...
                    <div className="relative z-10 w-full">
                        {/* HIDDEN FOR NOW */}
                        {/* {(!isComplete1 ||
                        (showSecondParagraph && !isComplete2)) && (
                        <div className="absolute -top-7 right-0 text-xs opacity-60 text-white">
                            Press any key or click text to skip
                        </div>
                        )} */}

                        <div className="flex flex-col font-rubik font-normal text-[1.188rem] text-white text-justify tracking-tight leading-[2rem]">
                        {/* First Paragraph */}
                        <div
                            className="w-full mb-3 cursor-pointer"
                            onClick={
                            !isComplete1 ? handleSkipFirstParagraph : undefined
                            }
                            onDoubleClick={
                            !isComplete1 ? handleSkipFirstParagraph : undefined
                            }
                        >
                            <p className="block transition-all duration-300 hover:opacity-90">
                            {displayText1}
                            {!isComplete1 && (
                                <span
                        ...
                        {showSecondParagraph && (
                            ...
                            onClick={
                                !isComplete2 ? handleSkipSecondParagraph : undefined
                            }
                            onDoubleClick={
                                !isComplete2 ? handleSkipSecondParagraph : undefined
                            }
                ...
                {/* Bottom Navigation */}
                <div className="flex justify-between items-center w-full sticky bottom-0 z-10">
                <NavigationArrow
                    direction="left"
